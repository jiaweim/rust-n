...
- [引用和借用](#引用和借用)
  ...
  - [3. 不可变引用](#3-不可变引用)
  - [4. 可变引用](#4-可变引用)
    - [4.1. 可变引用只能有一个](#41-可变引用只能有一个)
    - [4.2. 可变引用和不可变引用不能同时存在](#42-可变引用和不可变引用不能同时存在)
  - [5. 悬空引用](#5-悬空引用)
  - [6. ref](#6-ref)
  - [7. 借用规则总结](#7-借用规则总结)
  - [8. 更多示例](#8-更多示例)
...
仅仅通过转移所有权的方式获取值，会让程序变得很复杂。Rust 使用变量引用解决该问题，获取引用的操作称为**借用**。

## 2. 引用和解引用

常规引用类似指针，指向对象存储的内存地址。例如，创建一个 `i32` 值的引用 `y`，然后用解引用运算符 `*` 获得 `y` 引用的值：
...
不可变引用，又称为共享引用（shared reference），引用值不可变。
...
- 无需先通过函数参数传入所有权，再通过函数返回传入的所有权，代码更简洁
...
在老版本的编译器中（Rust 1.31 前）会报错，因为 `r1` 和 `r2` 的作用域在花括号 `}` 处结束，`r3` 的借用触发 **无法同时借用可变和不可变引用** 的规则。

但是在新编译器中，该代码将顺利通过，因为 **引用作用域的结束位置从花括号变成最后一次使用的位置**，因此 `r1` 借用和 `r2` 借用在 `println!` 后就结束了，此时 `r3` 可以顺利借用到可变引用。
...
因为 `s` 是在 `dangle` 函数内创建的，所以当 `dangle` 的代码执行完，`s` 的内存被释放。此时视图返回它的引用，导致引用指向一个无效 `String`。Rust 不会让这种事发生。
