...
此处生命周期标注仅仅说明，这两个参数 `first` 和 `second` 至少活得和 `'a` 一样久，至于到底活多久或者哪个活得更久，我们无法得知：
...
该函数的参数和返回值都是引用类型，尽管没有显式为其标注生命周期，编译依然可以通过。因为编译器为了简化使用，运用了生命周期消除。
...
2. 若**只有一个输入生命周期**（函数只有一个引用类型参数），那么该生命周期会被赋给所有输出生命周期，即所有返回值的生命周期都等于该输入的生命周期
...
编译器先根据规则一，为每个参数标注一个生命周期：

```rust
fn first_word<'a>(s: &'a str) -> &str { // 编译器自动为参数添加生命周期
```

然后，根据规则二，因为只有一个输入生命周期，因此输出生命周期与该输入生命周期相同：
...
- 方法签名中一般不需要标注生命周期，得益于生命周期消除的第一和第三规则
