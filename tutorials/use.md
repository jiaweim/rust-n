...
- [use](#use)
  ...
  - [引入项再导出](#引入项再导出)
  - [使用第三方包](#使用第三方包)
    - [crates.io, lib.rs](#cratesio-librs)
  - [使用 {} 简化引入方式](#使用--简化引入方式)
    - [self](#self)
...
如 `crate::front_of_house::hosting::add_to_waitlist` 这样的函数调用形式过于繁琐，可以使用 `use 关键字把路径提前引入到当前作用域中，随后的调用就可以省略该路径，简化代码。

## 基本引入方式

在 Rust 中，引入模块中的项有两种方式：绝对路径和相对路径，先来看看使用绝对路径的引入方式。
...
这里使用 `use` 和绝对路径的方式，将 `hosting` 模块引入到当前作用域中，然后只需通过 `hosting::add_to_waitlist` 即可调用目标模块中的函数，相比 `crate::front_of_house::hosting::add_to_waitlist()` 要简单的多，那么还能更简单吗？

### 相对路径引入模块中的函数

使用相对路径进行引入直接引入该模块中的 `add_to_waitlist` 函数：
...
从简洁性来说，引入函数自然是更甚一筹，但是在某些时候，引入模块会更好：
...
其实严格来说，对于引用方式并没有需要遵守的惯例，主要还是取决于你的喜好，建议：优先使用最细粒度(引入函数、结构体等)的引用方式，如果引起了某种麻烦(例如前面两种情况)，再使用引入模块的方式。
...
当外部的模块项 `A` 被引入到当前模块中时，它的可见性自动被设置为私有，如果希望其它外部代码引用我们的模块项 `A`，可以对它进行再导出：
...
例如，在 Rust 中，包是一个模块树，可以通过 `pub(crate) item;` 这种方式来实现：`item` 虽然是对外可见的，但是只在当前包内可见，外部包无法引用到该 `item`。
